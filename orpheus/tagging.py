...
def tag_file(file_path: str, image_path: str, track_info: TrackInfo, credits_list: list, embedded_lyrics: str, container: ContainerEnum):
    ...
    if track_info.tags.md5_hash:
        if container == ContainerEnum.m4a:
            tagger.RegisterTextKey('md5', '----:com.apple.itunes:MD5')
            tagger['md5'] = track_info.tags.md5_hash.encode()
        elif container == ContainerEnum.mp3:
            tagger.tags.RegisterTXXXKey('MD5', 'MD5')
            tagger['MD5'] = track_info.tags.md5_hash
        else:  # FLAC, OGG, OPUS
            tagger['MD5'] = track_info.tags.md5_hash
