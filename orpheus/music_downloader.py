...
class Downloader:
    ...
    def download_track(self, track_id, album_location='', main_artist='', track_index=0, number_of_tracks=0, cover_temp_location='', indent_level=1, m3u_playlist=None, extra_kwargs={}):
        ...
        # Calculate MD5 hash for the final audio file if enabled
        if self.global_settings['advanced']['calculate_md5_hash']:
            self.print('Calculating MD5 hash')
            try:
                md5_hash = calculate_file_md5(track_location)
        ...
        try:
            ...
            if old_track_location:
                # Calculate MD5 for the original file if it exists and MD5 is enabled
                if self.global_settings['advanced']['calculate_md5_hash']:
                    try:
                        old_md5_hash = calculate_file_md5(old_track_location)
                        old_track_info = TrackInfo(
                            name=track_info.name,
                            album=track_info.album,
                            album_id=track_info.album_id,
                            artists=track_info.artists,
                            tags=Tags(**asdict(track_info.tags), md5_hash=old_md5_hash),
                            codec=track_info.codec,
                            ...
                            animated_cover_url=track_info.animated_cover_url,
                            description=track_info.description,
                            bit_depth=track_info.bit_depth,
                            sample_rate=track_info.sample_rate,
                            bitrate=track_info.bitrate,
                            download_extra_kwargs=track_info.download_extra_kwargs,
                            cover_extra_kwargs=track_info.cover_extra_kwargs,
                            credits_extra_kwargs=track_info.credits_extra_kwargs,
                            lyrics_extra_kwargs=track_info.lyrics_extra_kwargs,
                        ...
                        self.print(f'Warning: Could not calculate MD5 hash for original file: {e}')
                        tag_file(old_track_location, cover_temp_location if self.global_settings['covers']['embed_cover'] else None,
